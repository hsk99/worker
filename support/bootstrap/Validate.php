...
class Validate
{
            ...
            if (false === $result) {
                ...
                if (isset($msg[$i])) {
                    ...
                    if (is_string($message) && strpos($message, '{%') === 0) {
                        $message = self::$typeMsgZh[substr($message, 2, -1)];
                    }
                } else {
    ...
    protected function getRuleMsg($attribute, $title, $type, $rule)
    {
        if (isset($this->message[$attribute . '.' . $type])) {
            ...
            $msg = $title . self::$typeMsgZh['not conform to the rules'];
        }

        if (is_string($msg) && 0 === strpos($msg, '{%')) {
            $msg = self::$typeMsgZh[substr($msg, 2, -1)];
        } elseif (!empty(self::$typeMsgZh[$msg])) {
            $msg = self::$typeMsgZh[$msg];
        }
